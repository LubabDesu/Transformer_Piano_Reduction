...
def train_model(model, train_loader, val_loader, num_epochs = 100, device='cpu') :
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, batch in enumerate(train_loader):
            ...
            # print("im in the batch for loop\n")
            ...
            # loss_onset = losses['loss/onset']
            # loss_offset = losses['loss/offset']
            # loss_frame = losses['loss/frame']
            # loss = (18 * loss_onset + loss_offset + loss_frame) / 20
            # print("Calculated losses!!!!!!\n")
            loss = loss_calc.onsetFrameLoss(batch, predictions=predictions)
        ...
        # print(f"  → Epoch {epoch+1}, Current loss : {epoch_loss}")
        ...
        with torch.no_grad() : 
            for batch in val_loader:
                ...
                # val_losses = loss_calc.onsetFrameLoss(batch, predictions=outputs)
                # val_loss_onset = val_losses['loss/onset']
                # val_loss_offset = val_losses['loss/offset']
                # val_loss_frame = val_losses['loss/frame']
                # val_loss = (18 * val_loss_onset + val_loss_offset + val_loss_frame) / 20
                val_loss = loss_calc.onsetFrameLoss(batch, predictions=outputs)
